...
int search(vector<char>a, int* b, int location, int max) {
	...
	if (  (location == max) || ( (max - location == 1) && a[max-1] != inp )) {
		cout << "THE INPUT CANNOT BE FOUND" << endl;
		return -1;
	}
	...
	if (a[location + ((max - location) / 2)] - '0' == inp) {
		...
		cout << "THE INPUT WAS FOUND IN THE ARRAY AT  LOCATION  :: " << answer  << endl;
		return 1;
	}
	...
	if (a[(max - location) / 2] - '0' == inp) {
		...
		cout << "The input of the array was found   :: " << answer << endl;
		return 1;
	}
